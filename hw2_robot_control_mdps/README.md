...
1. If you increase the width of the Lemniscate (increasing a), what issue can happen with the robot performing IK?
    - if it is too big then the curve will be big enough so that the robot's arm cannot reach all the points, preventing it from finding the local optmimum under the error threshold, and never converging.
2. What can happen if you change the dt parameter in IK?
    - Since this is an iterative method with linear-approximations, if you set the dt parameter small it will take many small accurate steps, but could not converge below the error threshold after the max iterations, it might still require more iterations. If you set it big it can take less steps but can overshoot
3. We implemented a simple numerical IK solver. What are the advantages and disadvantages compared to an analytical IK solver?
    - numerical solvers can work in principle with any robot geometry without specific equations making it simpler to implement. 
      - On the other hand these methods are usually slower since they are iterative
      - don't have convergence guarantees
      - might get stuck in local minima
      - and starting conditions might lead to different solutions
4. What are the limits of our IK solver compared to state-of-the-art IK solvers?
    - checking the examples we can clearly see that our implementation does not handle obstacles 
    - does not handle limits or constraints in the joints
    - does not leverage second order information to break redundancies
    - and can only focus in a single task for the end-effector rather than adding for example elbow position constraints.
...
1. If you keep increasing $K_P$, what issue arises when tracking the waypoints?
  - Increasing the Kp gain will cause the robot to pull too aggressively toward the target, leading to severe overshooting and uncontrollable oscillations around the waypoints
2. How does $K_D$ mitigate the effect you saw above when increasing $K_P$?
- The derivative gain will acts as a physical damper by monitoring the rate of change of the error and applying a braking force as the arm rapidly approaches the target, effectively absorbing and suppressing the overshoot caused by a high KP
3. In what scenarios is a non-zero $K_I$ needed for the controller to perform well?
- the non-zero integral gain would be useful for scenarios with persistent external forces, for example gravity, and correct the small but steady errors.
