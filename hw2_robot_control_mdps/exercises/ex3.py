...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray:
    ...
    return (action + 1) / 2 * (upper_limits - lower_limits) + lower_limits
...
def get_obs(qpos: np.ndarray, ee_pos_w: np.ndarray, ee_rot_w: np.ndarray, base_pos_w: np.ndarray, base_rot_w: np.ndarray, target_pos_w: np.ndarray) -> np.ndarray:
    ...
    # Get the conjugate of the base quaternion (represents inverse rotation)
    ...
    # Using quaternion: q_conj * (pos_diff as pure quat) * q
    ee_pos_diff = ee_pos_w - base_pos_w
    ee_pos_quat = np.array([1.0, ee_pos_diff[0], ee_pos_diff[1], ee_pos_diff[2]])
    ee_pos_rotated = quat_mul(quat_mul(base_quat_conj, ee_pos_quat), base_quat_w)
    ee_pos_base = ee_pos_rotated[1:4]  # Extract xyz components
    
    # Transform end-effector quaternion to base frame:
    # ee_quat_base = q_base^(-1) * q_ee = q_base_conj * q_ee
    ee_quat_base = quat_mul(base_quat_conj, ee_quat_w)
    ee_quat_base = quat_normalize(ee_quat_base)
    
    # Transform target position to base frame (same as ee_pos)
    target_pos_diff = target_pos_w - base_pos_w
    target_pos_quat = np.array([1.0, target_pos_diff[0], target_pos_diff[1], target_pos_diff[2]])
    target_pos_rotated = quat_mul(quat_mul(base_quat_conj, target_pos_quat), base_quat_w)
    target_pos_base = target_pos_rotated[1:4]  # Extract xyz components
    
    # Concatenate all observations
    obs = np.concatenate([
        qpos,           # joint positions
        ee_pos_base,    # end-effector position in base frame (3,)
        ee_quat_base,   # end-effector quaternion in base frame (4,)
        target_pos_base # target position in base frame (3,)
    ])
