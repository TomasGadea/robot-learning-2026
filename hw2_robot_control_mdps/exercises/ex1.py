...
def ik_track(model, data, site_name, target_pos,
    ...
    for i in range(max_iters):
        ...
        if np.linalg.norm(err_pos) < 1e-3:
            print(f"Target reached within threshold in {i} iterations.")
            break
        ...
        linear_system = J @ J.T + damping * np.eye(6)
        weighted_err = np.hstack([pos_gain * err_pos, np.zeros(3)])
        x = np.linalg.solve(a=linear_system, b=weighted_err) # note to self: ax = b
        qdot = J.T @ x
